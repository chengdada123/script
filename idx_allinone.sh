...
load_vm_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        unset DISK_SIZE MEMORY CPUS HOST_REMOTE_PORT GUEST_REMOTE_PORT GUI_MODE PORT_FORWARDS IMG_FILE SEED_FILE CREATED
...
MEMORY="$MEMORY"
CPUS="$CPUS"
HOST_REMOTE_PORT="$HOST_REMOTE_PORT"
GUEST_REMOTE_PORT="$GUEST_REMOTE_PORT"
GUI_MODE="$GUI_MODE"
...
create_new_vm() {
    ...
    # Set GUEST_REMOTE_PORT based on OS_TYPE
    if [[ "$OS_TYPE" == "windows" ]]; then
        GUEST_REMOTE_PORT=3389
    ...
    while true; do
        read -p "$(print_status "INPUT" "$remote_prompt")" HOST_REMOTE_PORT
        HOST_REMOTE_PORT="${HOST_REMOTE_PORT:-$default_host_remote_port}"
        if validate_input "port" "$HOST_REMOTE_PORT"; then
            # Check if port is already in use
            if ss -tln 2>/dev/null | grep -q ":$HOST_REMOTE_PORT "; then
                print_status "ERROR" "Port $HOST_REMOTE_PORT is already in use"
            else
                break
            fi
        fi
    ...
    while true; do
        local current_gui=$GUI_MODE
        read -p "$(print_status "INPUT" "Enable GUI mode? (y/n, default: ${GUI_MODE,,}): ")" gui_input
        gui_input="${gui_input:-${GUI_MODE,,}}"
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        if [[ "$OS_TYPE" != "windows" ]]; then
            print_status "INFO" "SSH: ssh -p $HOST_REMOTE_PORT $USERNAME@localhost"
            print_status "INFO" "Password: $PASSWORD"
        else
            print_status "INFO" "RDP: Connect to localhost:$HOST_REMOTE_PORT"
        ...
        # Check if seed file exists for non-Windows
        ...
        # Add seed drive for non-Windows
        if [[ "$OS_TYPE" != "windows" ]]; then
...
show_vm_info() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        echo "Host Remote Port: $HOST_REMOTE_PORT"
        echo "Guest Remote Port: $GUEST_REMOTE_PORT"
...
edit_vm_config() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        local remote_prompt
        if [[ "$OS_TYPE" == "windows" ]]; then
            remote_prompt="RDP Port"
        else
            remote_prompt="SSH Port"
        fi
        ...
        while true; do
            ...
            case $edit_choice in
                ...
                4)
                    while true; do
                        read -p "$(print_status "INPUT" "Enter new $remote_prompt (current: $HOST_REMOTE_PORT): ")" new_host_remote_port
                        new_host_remote_port="${new_host_remote_port:-$HOST_REMOTE_PORT}"
                        if validate_input "port" "$new_host_remote_port"; then
                            # Check if port is already in use
                            if [ "$new_host_remote_port" != "$HOST_REMOTE_PORT" ] && ss -tln 2>/dev/null | grep -q ":$new_host_remote_port "; then
                                print_status "ERROR" "Port $new_host_remote_port is already in use"
                            else
                                HOST_REMOTE_PORT="$new_host_remote_port"
                                break
                            fi
                        fi
                    done
            ...
            # Recreate seed image with new configuration if user/password/hostname changed (for non-Windows)
            if [[ "$OS_TYPE" != "windows" ]] && [[ "$edit_choice" -eq 1 || "$edit_choice" -eq 2 || "$edit_choice" -eq 3 ]]; then
